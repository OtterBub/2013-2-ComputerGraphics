...
void Target::moveReversing(bool _auto) {
	m_autoReverse = _auto;
}
...
bool Pillar::pickingCheck(TKpoint pick) {
	...
	if( ((abs(m_viewPosition.x - pick.x) < 35) && 
		(abs((m_viewPosition.y + 100) - pick.y) < 100) &&
		(abs((m_viewPosition.z) - pick.z) < 40)) ){
			return true;
	}
	return false;
}
...
bool IronBarrel::pickingCheck(TKpoint pick) {
	if( ((abs(m_viewPosition.x - pick.x) < 60) && 
		(abs(m_viewPosition.y - pick.y) < 60)) &&
		(abs(m_viewPosition.z - pick.z) < 60) ){
			return true;
	}
	return false;
}