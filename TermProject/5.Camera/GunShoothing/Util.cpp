...
float getDist(TKpoint startPoint, TKpoint endPoint) 
{
	TKpoint dist;
	dist.x = (endPoint.x - startPoint.x );
	dist.y = (endPoint.y - startPoint.y );
	dist.z = (endPoint.z - startPoint.z );
...
TKpoint getUnitVector(TKpoint startPoint, TKpoint endPoint) 
{
	TKpoint result = {0,};
	...
	float scalar = getDist( startPoint, endPoint );
